...
@deprecated(version='0.1.0', reason="腾讯云爬取数据功能已失效，推荐使用其自用 API：https://cloud.tencent.com/document/api/242/56203")
class Qcloud(WhoisABC):
    ...
    def fetch(self, url, domain) -> Optional[Response]:
        '''
        请求数据
        :param url: 网址
        :param domain: 域名
    ...
    def query(self, domain)->QueryResult:
        ...
        try:
            ...
            if resp['code'] == "FailedOperation.DomainNotRegisterFailed": # 未注册
                ...
                raise ValueError(f"resp code {resp['code']}, message {resp['message']}")
        ...
        #     # {"code":"99999999","message":"验证失败","error":{"name":"RequestError","message":"验证失败"},"timestamp":"2025-07-21T07:50:38.488Z","tid":"4b82c314be6582c709b0e7f8873eea61"}
            ...
            raise ValueError(f'{self.provider_name} find domain: {domain}, err:{e}')

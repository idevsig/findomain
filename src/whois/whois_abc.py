...
class WhoisABC(ABC):
    ...
    def supported_suffixes(self) -> list[str]:
        """
        Returns a list of domain suffixes supported by this WHOIS provider.
        """
        return []
    ...
    def supported_suffixes(self, value: list[str]):
        ...
        if not all(isinstance(item, str) and len(item) > 1 for item in value):
            raise TypeError("All items in supported_suffixes list must be strings.")
    ...
    def _is_service_available(self) -> bool:
        ...
        try:
            if not self.base_host:
                raise ValueError("Error: base_host is empty, cannot check service availability.")

            socket.gethostbyname(self.base_host)
            return True
        except socket.gaierror as e:
            # Use 'e' only within this block
            raise ValueError(f"DNS resolution failed for {self.base_host}: {e}")
        except Exception as e:
            # Use 'e' only within this block
            raise ValueError(f"An unexpected error occurred during service availability check: {e}")
    ...
    def supported(self, suffix):
        ...
        try:
            ...
            raise ValueError(f'Error: ({self.provider_name}) this suffix is not supported: {suffix}')
    ...
    @abstractmethod
    def fetch(self) -> Optional[Response]:
        '''
        请求数据
        '''

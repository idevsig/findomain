...
class App:
    ...
    def domain_from_url(self, url):
        ...
        try:
            ...
            raise ValueError(f'Get domain info from url {url} failed: {e}.') from e
    ...
    def filter_dnps(self, dnp_arr) -> list[WhoisABC]:
        '''
        筛选域名提供商
        '''
        # 配置中的 ISPS 遍历
...
#             print(f'Update domain info failed.')
    ...
    def fetch(self):
        ...
        # 从配置中获取最大重试次数
        if isinstance(self.config.setting.max_retries, int) and self.config.setting.max_retries >= 3:
            ...
            while retries < max_retries:
                try:
                    ...
                    if not result.is_success:  # 错误码不为0
                        ...
                        raise ValueError(f'{dnp.provider_name} fetch domain {domain} failed, err: {e}, retries: {retries}') from e  # 错误日志
                    ...
                    # 增加重试次数
                    retries += 1
                    logging.warning(f'Fetch err: {e}, retries: {retries}, dnp: {dnp.provider_name}')
                    if retries >= max_retries:
                        logging.error(f'Max retries reached fetch domain {domain} info, exiting loop.')
                        # 设置标志以跳出最外层的for循环
                        should_break = True
                        break  # 达到最大重试次数，退出循环
                    continue
    ...
    def save_domain_list(self):
        ...
        保存域名列表
        '''
        save_path = os.path.join(self.config.setting.log_dir, self.config.setting.domain_file)
    ...
    def save_result_csv(self):
        ...
        with open(self.result_file, 'w') as f:
            f.write('domain, available, registration_date, expiration_date, error_code, provider\n')
    ...
    def upload_result_csv(self):
        ...
        '''
        # 检查 transfer 是否配置
        if not self.config.setting.transfer:
            return []

        # 检查 transfer 是否以 http 或 https 开头
        if not self.config.setting.transfer.startswith('http') and not self.config.setting.transfer.startswith('https'):
        ...
        try:
            with open(self.result_file, 'rb') as file:
                req = HttpRequest()
                req.put(self.config.setting.transfer, data = file)
                if req.response.status_code != 200:
                    raise ValueError(f'Upload result file failed, status code {req.response.status_code}')

                upload_info.append(req.text)

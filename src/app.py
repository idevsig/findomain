...
class App:
    ...
    def domain_from_url(self, url):
        ...
        try:
            ...
            if req.response.status_code != 200:
                raise ValueError(
                    f'Get domain info status code {req.response.status_code}'
                )
    ...
    def startchar_from_url(self):
        if self.config.domain.resume_url and is_valid_url(
            self.config.domain.resume_url
        ):
            try:
                req = HttpRequest()
                req.update_headers({})
                req.get(self.config.domain.resume_url)

                # 获取失败
                if req.response.status_code != 200:
                    raise ValueError(f'status code {req.response.status_code}')

                data = json5.loads(req.text)
                # 提取上次查询的域名信息
                if data and 'last_query_domain' in data:
                    # 拆分域名为前缀和后缀
                    domain, *rest = data['last_query_domain'].split('.')
                    # 已完成
                    if domain == '-1':
                        raise ValueError('Query completed.')

                    # 校验域名长度
                    if domain and len(domain) != self.config.domain.length:
                        raise ValueError(
                            f'Invalid length: {domain}({len(domain)}), expect: {self.config.domain.length}',
                        )
                    self.config.domain.start_char = domain

                    # 校验后缀
                    if rest and rest[0] != self.config.domain.suffixes:
                        raise ValueError(
                            f'Invalid suffix: {rest[0]}, domain_suffix: {self.config.domain.suffixes}',
                        )
            except Exception as e:
                raise ValueError(f'Get start char from url: {e}') from e
    ...
    def domain_resume(self, domain):
        ...
        :param domain: 域名
        """
        if not self.config.domain.resume_url:
            return
        try:
            if not is_valid_url(self.config.domain.resume_url):
                raise ValueError(
                    f'Invalid url: {self.config.domain.resume_url}',
                )
            ...
            if req.response.status_code != 201:
                raise ValueError(
                    f'status code {req.response.status_code}',
                )
        except Exception as e:
            raise ValueError(f'Update resume info failed: {e}') from e
    ...
    def run(self, custom_domain=None):
        ...
        try:
            ...
            # 发送断点续传域名
            self.domain_resume(self.last_failed_domain)
